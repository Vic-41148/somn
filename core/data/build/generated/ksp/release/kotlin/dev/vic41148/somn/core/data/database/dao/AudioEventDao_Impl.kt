...
public class AudioEventDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfAudioEventEntity = object : EntityInsertAdapter<AudioEventEntity>() {
      protected override fun createQuery(): String = "INSERT OR ABORT INTO `audio_events` (`id`,`sessionId`,`timestampMillis`,`durationSeconds`,`type`,`intensityDecibels`) VALUES (nullif(?, 0),?,?,?,?,?)"
    ...
    return createFlow(__db, false, arrayOf("audio_events")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = AudioEventEntity(_tmpId,_tmpSessionId,_tmpTimestampMillis,_tmpDurationSeconds,_tmpType,_tmpIntensityDecibels)
          _result.add(_item)
        }
        _result
      } finally {
