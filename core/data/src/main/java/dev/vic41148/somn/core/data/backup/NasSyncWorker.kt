...
class NasSyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            // Same wrong-filename bug as BackupRepository.performSilentBackup(): the real Room
            // DB file is SleepDatabase.DATABASE_NAME, not "somn-database" — this always resolved
            // to a nonexistent file, so the remote DB snapshot upload silently never ran either.
            val dbFile = applicationContext.getDatabasePath(SleepDatabase.DATABASE_NAME)
            if (dbFile.exists()) {
