...
object YamnetLabels {
    ...
    /** Speech-family classes that indicate someone talking near the phone. */
    private val TALK_CLASSES = setOf(
        "Speech", "Child speech, kid speaking", "Conversation", "Narration, monologue",
        "Babbling", "Speech synthesizer", "Shout", "Bellow", "Whoop", "Yell",
        "Children shouting", "Screaming", "Whispering", "Hubbub, speech noise, speech babble"
    )

    /** Snoring-family classes. */
    private val SNORE_CLASSES = setOf(
        "Snoring", "Snort"
    )

    /** Cough-family classes. */
    private val COUGH_CLASSES = setOf(
     ...
     */
    private val ANOMALY_CLASSES = setOf(
        "Breathing", "Wheeze", "Gasp", "Sigh", "Groan", "Grunt", "Sniff", "Sneeze"
    )
    ...
    fun classNameToAudioEventType(className: String): AudioEventType? = when (className) {
        in TALK_CLASSES -> AudioEventType.TALK
        in SNORE_CLASSES -> AudioEventType.SNORE
        in COUGH_CLASSES -> AudioEventType.COUGH
        in ANOMALY_CLASSES -> AudioEventType.ANOMALY
        else -> null
    }

    // 521 AudioSet class names, index-aligned with yamnet.tflite's output tensor (verified
    // against the label list embedded in the model file's own metadata zip).
