...
- **CI fixes.** JDK pin moved out of the repo `gradle.properties` (it broke every GitHub-managed workflow), release attestations got their missing permission, Gitleaks got its token, and SDK-37-only deps are ignored by Dependabot until the bump.
...
- **Pre-release pipeline, first slice.** CI now assembles the minified standalone release (the compileSdk 35/36 split proved debug-green means nothing), runs release lint alongside debug lint, and fails only on CVEs whose classes are actually in the shipped DEX (`scripts/verify-shipped-cve.sh` over the CycloneDX BOM — today's scan: 16 BOM entries, all build-time Netty/Bouncy Castle/HttpClient, zero in the APK). Debug builds gain StrictMode tripwires (main-thread IO, leaked SQLite/closables, penaltyLog only). Gitleaks history scan clean across 218 commits; MobSF static scan triaged (task-hijacking flag is a false positive at targetSdk 36, no hard-coded keys in real source).
