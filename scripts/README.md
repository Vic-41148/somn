...
| `run-release-preflight.sh [--boot-only] [--avd NAME]` | THE release gate: boots the emulator detached with the 1080x2340 phone skin (reusing an already-correct one, rebooting a wrong-size one), then runs all 15 device scripts sequentially and prints a pass/fail matrix. `--boot-only` just ensures the emulator is up. |
