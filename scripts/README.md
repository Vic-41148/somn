...
| `run-release-preflight.sh [--boot-only] [--avd NAME]` | THE release gate: boots the emulator detached with the 1080x2340 phone skin (reusing an already-correct one, rebooting a wrong-size one), then runs all 15 device scripts sequentially and prints a pass/fail matrix. `--boot-only` just ensures the emulator is up. Launch it as `./scripts/run-release-preflight.sh` — direct execution self-detaches (the shebang), so no launcher can kill it. |
| `run-session-e2e.sh` | THE morning-alert test: starts a real tracking session via the UI, stops it, asserts the session completed, deep-sleep alert (1004) + Luteal alert (1003) posted, FGS stopped. The 1004 check is data-driven — it compares against the session's actual deep % (id 1001 was never safe: it collided with the tracking FGS notification id, and `stopForeground(REMOVE)` at teardown silently deleted the alert). Options: `--profile default\|cycling`, `--no-seed`, `--duration N`, `--retries N`. |
